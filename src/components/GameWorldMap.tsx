...
export const GameWorldMap: React.FC = () => {
  ...
  return (
    ...
    >
       ...
       <div className="absolute top-20 md:top-24 left-0 w-full px-6 md:px-10 z-20 flex items-start justify-between gap-2 pointer-events-none">
          {/* Back Button */}
          <div className="pointer-events-auto shrink-0">
            <button onClick={() => setGameState('SKILL_MATRIX')} className={`premium-button px-4 py-2.5 md:px-6 md:py-3 text-sm flex items-center justify-center gap-2 border transition-all ${isLightMode ? 'bg-white text-slate-900 border-slate-200 hover:bg-slate-50' : 'bg-slate-900 text-slate-100 border-slate-700 hover:bg-slate-800'}`}>
               <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M19 12H5M12 19l-7-7 7-7"/></svg>
               <span className="hidden md:inline">{t("map.backToProfile")}</span>
            </button>
          </div>

          {/* Titles */}
          <div className="text-right pointer-events-none flex flex-col items-end">
            <h2 className={`text-xl md:text-3xl font-extrabold tracking-tight px-4 py-2 rounded-xl backdrop-blur-sm transition-colors ${isLightMode ? 'text-slate-900 bg-white/80' : 'text-slate-100 bg-slate-900/80'}`}>{t("map.projectJourney")}</h2>
            <p className={`font-bold tracking-widest mt-2 uppercase text-[10px] md:text-xs px-4 py-1.5 rounded-xl inline-block backdrop-blur-sm transition-colors ${isLightMode ? 'text-orange-600 bg-white/80' : 'text-orange-400 bg-slate-900/80'}`}>{t("map.exploreCaseStudies")}</p>
          </div>
       </div>
       ...
       >
          ...
          <div className="relative w-[1000px] h-[800px]">
             ...
             <svg className="absolute inset-0 w-full h-full pointer-events-none">
                ...
                {/* Cluster B — AI Architecture: 07↔08 */}
                <path d={`M ${CV_PROJECTS[6].x} ${CV_PROJECTS[6].y} L ${CV_PROJECTS[7].x} ${CV_PROJECTS[7].y}`} stroke={isLightMode ? "rgba(15,23,42,0.2)" : "rgba(255,255,255,0.2)"} strokeWidth="3" strokeDasharray="6,6" />
                {/* Cluster C — Process & Vision: 05↔06, 06↔09 */}
                ...
                <path d={`M ${CV_PROJECTS[5].x} ${CV_PROJECTS[5].y} L ${CV_PROJECTS[8].x} ${CV_PROJECTS[8].y}`} stroke={isLightMode ? "rgba(15,23,42,0.2)" : "rgba(255,255,255,0.2)"} strokeWidth="3" strokeDasharray="6,6" />
                {/* Inter-cluster: 07→01 (AI→Product), 08→05 (AI→Process), 03→05 (Product→Process) */}
                <path d={`M ${CV_PROJECTS[6].x} ${CV_PROJECTS[6].y} L ${CV_PROJECTS[0].x} ${CV_PROJECTS[0].y}`} stroke={isLightMode ? "rgba(15,23,42,0.2)" : "rgba(255,255,255,0.2)"} strokeWidth="3" strokeDasharray="6,6" />
                <path d={`M ${CV_PROJECTS[7].x} ${CV_PROJECTS[7].y} L ${CV_PROJECTS[4].x} ${CV_PROJECTS[4].y}`} stroke={isLightMode ? "rgba(15,23,42,0.2)" : "rgba(255,255,255,0.2)"} strokeWidth="3" strokeDasharray="6,6" />
