          ...
          <div className={`flex justify-between items-start mb-10 border-b pb-8 relative z-10 ${isLightMode ? 'border-slate-200' : 'border-slate-700'}`}>
             <div>
                <div className="flex flex-wrap items-center gap-3 mb-4">
                   <span className={`px-4 py-1.5 rounded-full text-[10px] uppercase font-bold tracking-widest border ${isLightMode ? 'bg-orange-50 text-orange-700 border-orange-100' : 'bg-orange-950/50 text-orange-400 border-orange-500/20'}`}>{t(project.category)}</span>
                   <span className={`uppercase font-bold text-[10px] tracking-widest ${isLightMode ? 'text-slate-500' : 'text-slate-400'}`}>{t("quest.role")}: {t(project.role)}</span>
                </div>
                <h2 className={`text-3xl md:text-5xl font-extrabold tracking-tight ${isLightMode ? 'text-slate-900' : 'text-slate-100'} mb-4`}>{t(project.title)}</h2>
                {selectedQuest && (
          ...
          <div className={`space-y-10 relative z-10 ${isLightMode ? 'text-slate-600' : 'text-slate-300'}`}>
             ...
             <div>
                <h3 className={`text-xl mb-4 flex items-center gap-3 font-extrabold ${isLightMode ? 'text-slate-900' : 'text-slate-100'}`}>
                  ...
                  {t("quest.contextChallenge")}
                </h3>
                <p className="text-sm md:text-base leading-relaxed">{t(project.context)}</p>
             </div>
                 ...
                 <div className="flex flex-wrap gap-4 md:gap-6">
                   {project.results.map((res, idx) => (
                      <div key={idx} className={`px-5 py-4 rounded-2xl border flex flex-col flex-1 min-w-[140px] md:min-w-[160px] transition-colors ${isLightMode ? 'bg-white/60 border-white shadow-sm' : 'bg-slate-900/60 border-slate-700 shadow-sm'}`}>
                         <span className={`text-[10px] uppercase tracking-widest font-bold mb-1 ${isLightMode ? 'text-slate-500' : 'text-slate-400'}`}>{t(res.label)}</span>
                         <span className={`text-xl md:text-2xl font-black ${isLightMode ? 'text-amber-600' : 'text-amber-400'}`}>{t(res.value)}</span>
                      </div>
                   ))}
                </div>
             </div>
