...
export const DesktopHeader: React.FC<DesktopHeaderProps> = ({ className = '', onLogoClick }) => {
  ...
  return (
    <div className={`absolute top-2 left-0 w-full px-3 md:top-8 md:px-8 z-50 flex items-center justify-between pointer-events-none transform-style-preserve-3d translate-z-10 h-12 md:h-auto ${className}`}>
      ...
      <div className="flex items-center gap-2 pointer-events-auto">
        <LanguageToggle />
        <div className={`flex items-center h-10 md:h-[42px] overflow-hidden rounded-lg p-0.5 border shadow-[0_5px_15px_rgba(0,0,0,0.5)] transition-all ${isLightMode ? 'bg-slate-100 border-slate-200 backdrop-blur-md' : 'bg-slate-800/80 border-slate-700 backdrop-blur-md'}`}>
          ...
          <button 
            ...
            className={`w-10 h-full flex items-center justify-center rounded-md cursor-pointer transition-colors ${isLightMode ? 'text-orange-600 hover:bg-white' : 'text-slate-300 hover:text-white hover:bg-slate-700'}`}
